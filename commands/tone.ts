...
export const translate = async (
  ...
  export const analyzeTone = async (
    ...
    if (text && "")
      throw new Error("Could not get a text to analyze");
    ...
    return ToneAnalyzer.tone(toneParams).then((res) => {
        res.result.document_tone.tones?.sort((value1,value2)=> value2.score-value1.score);// orders the array to have the strongest emotion first and weaks last.
        res.result.document_tone.tones?.forEach(function (value){
            emotionList.push(`${value.tone_name}`);
